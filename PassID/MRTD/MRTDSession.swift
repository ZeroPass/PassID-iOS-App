...
//  MRTDSession.swift
//
// Modified copy of: https://github.com/AndyQ/NFCPassportReader/blob/ccb7a7940df514b2c8e4973aa59ed8b8024ac739/Sources/NFCPassportReader/SecureMessaging.swift
...
public class MRTDSession {
    ...
    func protect(apdu : NFCISO7816APDU ) throws -> NFCISO7816APDU {
        ...
        let CC = ISO9797Alg3.mac(key: self.ksmac, msg: N)
        Log.verbose("\tCompute MAC over N with KSmac")
        Log.verbose("\t\tCC: %@", CC.hex())
        ...
        Log.debug("Constructed protected APDU")
        Log.debug("\tProtectedAPDU: %@", protectedAPDU.hex())
...
//        let newAPDUData : [UInt8] = [UInt8](cmdHeader[0..<4]) + intToBin(data.count) + data + [0x00]
    ...
    func unprotect(rapdu : ResponseAPDU ) throws -> ResponseAPDU {
        ...
        Log.debug("\tRAPDU: %@", rapduBin.hex())
        
        // DO'87'
        // Mandatory if data is returned, otherwise absent
        if rapduBin[0] == 0x87 {
            let (encDataLength, o) = try TLV.decodeLength([UInt8](rapduBin[1...]))
            offset = 1 + Int(o)
            
            if rapduBin[offset] != 0x1 {
                throw MRTDSessionError.D087Malformed
            }
        ...
        if rapduBin[offset] == 0x8E {
            ...
            Log.verbose("\tConcatenate SSC and %@ and add padding", tmp)
            Log.verbose("\t\tK: %@", K.hex())
            
            Log.verbose("\tCompute MAC with KSmac")
            let CCb = ISO9797Alg3.mac(key: self.ksmac, msg: K)
            Log.verbose("\t\tCC: %@", CCb.hex())
            ...
            throw MRTDSessionError.MissingMandatoryFields
        ...
        if do87Data.count > 0 {
            ...
            Log.verbose("\tDecryptedData: %@", data.hex())
        }
        
        Log.debug("Unprotected APDU: [%@] sw1: %X sw2: %X", data.hex(), sw1, sw2 )
    ...
    func maskClassAndPad(apdu : NFCISO7816APDU ) -> [UInt8] {
        ...
        let res = pad([apdu.instructionClass | 0x0c, apdu.instructionCode, apdu.p1Parameter, apdu.p2Parameter])
        Log.verbose("\tCmdHeader: %@", res.hex())
        return res
    }
    
    func buildD087(apdu : NFCISO7816APDU) throws -> [UInt8] {
        let cipher = [0x01] + self.padAndEncryptData(apdu)
        let res = try [0x87] + TLV.encodeLength(cipher.count) + cipher
        Log.verbose("Build DO'87")
        Log.verbose("\tDO87: %@", res.hex())
        return res
    }
    ...
    func padAndEncryptData(_ apdu : NFCISO7816APDU) -> [UInt8] {
        ...
        Log.verbose("\tData: %@", paddedData.hex())
        Log.verbose("Encrypt data with KSenc")
        Log.verbose("\tEncryptedData: %@", enc.hex())
        return enc
    }
...
//        return res
    ...
    func buildD097(apdu : NFCISO7816APDU) throws -> [UInt8] {
        ...
        let res : [UInt8] = try [0x97] + TLV.encodeLength(binLe.count) + binLe
        Log.verbose("Build DO'97")
        Log.verbose("\tDO97: %@", res.hex())
        return res
    }
}
